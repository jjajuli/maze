...
class MazeGame(tk.Tk):
    def __init__(self, rows: int = 12, cols: int = 16, cell_size: int = 36):
        ...
        self.reset_btn = tk.Button(self, text="Nuevo laberinto", command=self.new_maze)
    ...
    def generate_path(self):
        """Genera un único camino (sin paredes): celdas válidas suspendidas en el vacío.
        Se usa DFS aleatorio para garantizar que Goal sea alcanzable.
        """
        self.path = self._dfs_path(self.start, self.goal)
        # En casos extremadamente raros, intentar varias veces
        tries = 0
        while not self.path and tries < 10:
            tries += 1
            self.path = self._dfs_path(self.start, self.goal)
        if not self.path:
            # Fallback: camino recto en L
            self.path = []
            r, c = self.start
            while c < self.goal[1]:
    ...
    def register_fall(self):
        self.falls += 1
        self.update_falls_label()
        # pequeño parpadeo para feedback
...
def main():
    # Puedes ajustar el tamaño aquí si quieres un tablero más grande/pequeño.
    app = MazeGame(rows=4, cols=5, cell_size=36)
    app.mainloop()
